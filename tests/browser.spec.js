...
test('phone-only practice: drag updates TV aim, release fires, bot returns turn', async ({ page, browser }) => {
  ...
  try {
    ...
    await expect(page.locator('#scene')).toHaveAttribute('data-aim-impact', /^(block|resident|fuelBarrel|bouncePad|firePlank|glassTrap|rockFall|magnet|ground|out)$/);
...
test('landscape controllers: orientation, cancel, mirrored pull, turns, reconnect', async ({ page, browser }) => {
  ...
  try {
    ...
    await cancel(a);
    // Pulling upward can now aim below the horizon for a nearby lower target.
    await pullStart(a, -30, -75);
    await expect.poll(() => a.phone.locator('#pull-angle').textContent().then(Number)).toBeLessThan(0);
...
test('harbor heist exposes attacker roles, two seals and live mission HUD', async ({ page, browser }) => {
  const code = await createRoom(page);
  const player = await joinPhone(browser, code, 'Yểm trợ');
  try {
    await page.locator('#ruleset-picker [data-ruleset="control"]').click();
    await expect(page.locator('#map-picker [data-map="harbor"]')).toHaveAttribute('aria-pressed', 'true');
    await expect(player.phone.locator('#ruleset-summary')).toContainText('phá 2 khóa');
    await expect(player.phone.getByRole('button', { name: 'Phe Đột kích' })).toBeVisible();
    await page.locator('#practice').click();
    ...
    await expect(player.phone.locator('.move-btn.control-node')).toHaveCount(1);
    ...
    await expect(player.phone.getByRole('button', { name: /Đặt thuốc nổ · Khóa cầu cảng/ })).toBeVisible({ timeout: 20_000 });
    await player.phone.getByRole('button', { name: /Đặt thuốc nổ · Khóa cầu cảng/ }).click();
    await expect(page.locator('#control-status')).toContainText('💣 1 ĐANG ĐẾM');
    await expect(page.locator('#scene')).toHaveAttribute('data-control-progress', '2-0/2:breach');

  } finally { await player.context.close(); }
});

test('four classic maps: shared selection, correct 3D geometry, replay, random and slow preview', async ({ page, browser }) => {
  ...
  let releasePreview;
  try {
    await expect(page.locator('#ruleset-picker [data-ruleset="classic"]')).toHaveAttribute('aria-pressed', 'true');
    for (const [mapId, name] of [['townhouse', 'Nhà phố'], ['tower', 'Tháp cao'], ['bridge', 'Cầu trên không'], ['fortress', 'Pháo đài']]) {
      ...
      await expect(page.locator('#scene')).toHaveAttribute('data-map', mapId);
      await expect(page.locator('#scene')).toHaveAttribute('data-ruleset', 'classic');
