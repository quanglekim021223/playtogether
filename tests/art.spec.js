...
test('environment meshes render and a fuel blast produces bounded effects and audio', async ({ page }) => {
  ...
  await expect(page.locator('#scene')).toHaveAttribute('data-headland-quality', /pbr-radial-(balanced|high)/);
  await expect(page.locator('#scene')).toHaveAttribute('data-headland-scatter', /^(72|124)$/);
  await expect(page.locator('#scene')).toHaveAttribute('data-architecture-quality', 'mediterranean-pbr-instanced');
  ...
  expect(secondMotion.waveTime).toBeGreaterThan(firstMotion.waveTime);
  expect(secondMotion.boatX).not.toBe(firstMotion.boatX);
  expect(secondMotion.cloudX).not.toBe(firstMotion.cloudX);
  await page.emulateMedia({ reducedMotion: 'reduce' });
  await page.waitForTimeout(100);
  const pausedMotion = await page.evaluate(() => window.environmentScene.environmentMotion());
  await page.waitForTimeout(180);
  expect(await page.evaluate(() => window.environmentScene.environmentMotion())).toEqual(pausedMotion);
